...
const TextAnimation = () => {
  ...
  const checkMobileNumber = async (mobile) => {
    try {
      const response = await fetch(`https://signpostphonebook.in/test.php`, {
        method: "POST",
        headers: {
  ...
  const insertRecord = async (e) => {
    ...
    const Data = {
      ...
      landline: mylandLine,
      lcode: myLcode,
      date: localDateTime, // Include date/time
    };
    ...
    try {
      ...
      if (jsonResponse.Message) {
        ...
        // Fetch and display the total count of entries
        if (jsonResponse.totalEntries) {
          alert(`Total Entries: ${jsonResponse.totalEntries}`);
        }
